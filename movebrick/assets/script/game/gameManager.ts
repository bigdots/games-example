...
export class GameManager extends Component {
  ...
  onLoad() {
    ...
    if (
      ...
      !playerData.instance.playerInfo.createDate
    ) {
    ...
    clientEvent.on(
      gameConstants.CLIENTEVENT_LIST.RESTARTGAME,
      this._initGame,
      this
    )
  ...
  private setAllRoleBrickId() {
    if (
      this._isLoadBrickNum === gameConstants.BRICKSKIN_COUNT &&
  ...
  private _initGame() {
    ...
    clientEvent.dispatchEvent(
      gameConstants.CLIENTEVENT_LIST.CHANGECAMERATYPE,
      gameConstants.CAMERA_TYPE_LIST.READY
    )
    ...
    const aiWayPath =
      gameConstants.AIWAY_PATH_IN_RESOURCES +
      '/' +
      gameConstants.AIWAY_NAME +
      level
  ...
  private _loadMap(level: number) {
    ...
    for (let i in mapData) {
      ...
      if (
        itemName > gameConstants.CSV_MAP_ITEM_RANGE.BRICK_LAYER[0] &&
        itemName < gameConstants.CSV_MAP_ITEM_RANGE.BRICK_LAYER[1]
      ) {
        resourceUtil.loadModelRes('brick/brickInRoad').then((prefab: any) => {
          // return;
          const brickLayerData = gameConstants.CSV_BRICK_LAYER_DATA.get(
            itemName
          ) as Array<number>
          if (!brickLayerData) {
            //避免出现错误的砖块堆信息
          ...
          const addX1 = gameUtils.getDirectionOfDistanceX(
            gameConstants.BRICK_LAYER_ONCE_Z,
            eulRad
          )
          const addZ1 = gameUtils.getDirectionOfDistanceZ(
            gameConstants.BRICK_LAYER_ONCE_Z,
            eulRad
          )
          //正后方的xz坐标
          eulRad = (180 + nowEul.y) * macro.RAD
          const addX2 = gameUtils.getDirectionOfDistanceX(
            gameConstants.BRICK_LAYER_ONCE_Z,
            eulRad
          )
          const addZ2 = gameUtils.getDirectionOfDistanceZ(
            gameConstants.BRICK_LAYER_ONCE_Z,
            eulRad
          )
          /*
          ...
          for (let j = 0; j < brickLayerData.length; j++) {
            ...
            while (brickCount < nowCount) {
              ...
              this._loadBrick(
                prefab,
                nowPos.clone().add3f(addX1 * nowMul, nowY, addZ1 * nowMul),
                nowEul
              )
              this._loadBrick(
                prefab,
                nowPos.clone().add3f(addX2 * nowMul, nowY, addZ2 * nowMul),
                nowEul
              )
        ...
        itemName > gameConstants.CSV_MAP_ITEM_RANGE.ROAD[0] &&
  ...
  private _loadBrick(brickPre: Prefab, pos: Vec3, eul: Vec3) {
    const ndNowBrick = poolManager.instance.getNode(
      brickPre,
      this.ndMap
    ) as Node
    ...
    let nowRow = gameUtils.checkNowBrickIndex(pos.x, pos.z)
  ...
  private _loadRoad(itemData: any) {
    resourceUtil.loadModelRes('road/' + itemData.name).then((prefab: any) => {
      ...
      if (ndItem.name === gameConstants.CSV_MAP_ITEM_NAME.ROAD_END) {
        ...
        //终点线的碰撞体
        const colliderFinishLine = ndItem
          .getChildByName(gameConstants.CSV_MAP_ITEM_NAME.FINISH_LINE)!
  ...
  public initEndRewardCircle() {
    if (
      GameManager.ndRewardCircle.children.length ===
      gameConstants.REWARD_CIRCLR_COUNT - 1
    ) {
      this.reSetEndRewardCircle()
    } else {
      //x2开始 结尾地板算x1
      resourceUtil
        .loadModelRes('road/' + gameConstants.CSV_MAP_ITEM_NAME.ROAD_END_REWARD)
        .then((prefab: any) => {
          for (let i = 1; i < gameConstants.REWARD_CIRCLR_COUNT; i++) {
            const ndEndCircle = poolManager.instance.getNode(
              prefab,
              GameManager.ndRewardCircle
            ) as Node
            ...
            if (i + 1 > 9) {
              ...
              ndNewNum
                ...
                .setMaterial(this.matNumList[i - 10 + 1], 0)
            } else {
              const ndNum = ndEndCircle.children[2] //第二个子节点为数字 10以上的十位数位数
              ndNum
                .getComponent(MeshRenderer)!
                .setMaterial(this.matNumList[i + 1], 0)
            }
  ...
  private reSetEndRewardCircle() {
    ...
    startPos.subtract3f(
      //由于终点的锚点存在偏差 因此先计算出终点的具体坐标
      gameUtils.getDirectionOfDistanceX(
        gameConstants.REWARD_START_TRANSVERSE,
        addEulRad
      ),
      0,
      gameUtils.getDirectionOfDistanceZ(
        gameConstants.REWARD_START_TRANSVERSE,
        addEulRad
      )
    )
    ...
    const getRandomLongitudinal = () => {
      //纵向随机值——仅允许终点往后 因此随机值只存在正数
      return (
        interval +
        gameUtils.getRandomRange(gameConstants.REWARD_LONGITUDINAL_RANGE_HALF)
      )
    }
    const getRandomTransverse = () => {
      //横向随机值——存在左右区间的可能性 所以 随机值允许正负类型数值
      return gameUtils.getRandomRange(
        gameConstants.REWARD_TRANSVERSE_RANGE_HALF,
        true
      )
    }
    for (let i = 1; i < gameConstants.REWARD_CIRCLR_COUNT; i++) {
      ...
      let randomX = gameUtils.getDirectionOfDistanceX(
        getRandomLongitudinal(),
        addEulRad
      )
      let randomZ = gameUtils.getDirectionOfDistanceZ(
        getRandomLongitudinal(),
        addEulRad
      )
      ...
      if (i === gameConstants.REWARD_CIRCLR_COUNT - 1) {
        ...
        const nextEulRad =
          (GameManager.ndRoadEnd.eulerAngles.y + 90) * macro.RAD
        let offsetTransverseX = gameUtils.getDirectionOfDistanceX(
          getRandomTransverse(),
          nextEulRad
        )
        let offsetTransverseZ = gameUtils.getDirectionOfDistanceZ(
          getRandomTransverse(),
          nextEulRad
        )
