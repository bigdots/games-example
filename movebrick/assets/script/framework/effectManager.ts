...
import { _decorator, Component, Node, Prefab, AnimationComponent, ParticleSystemComponent, Vec3, find, isValid, AnimationState, AnimationClip, UITransformComponent, Vec2, instantiate } from 'cc';
...
export class EffectManager extends Component {
     ...
     */
    public playParticle(path: string, worPos: Vec3, recycleTime: number = 0, scale: number = 1, eulerAngles?: Vec3 | null, callback?: Function | null, isBanDestroy: boolean = false) {
     ...
     * @param {Function} [isBanDestroy=false] 是否销毁
     */
    public playParticleNotPool(path: string, worPos: Vec3, recycleTime: number = 0, scale: number = 1, eulerAngles?: Vec3 | null, callback?: Function | null, isBanDestroy: boolean = false) {
        resourceUtil.loadEffectRes(path).then((prefab: any) => {
    ...
    public getRunFastEff(ndParent: Node) {
        resourceUtil.loadEffectRes(gameConstants.EFFECT_LIST.RUNFAST).then((prefab: any) => {
            let ndEffect: Node = poolManager.instance.getNode(prefab as Prefab, ndParent) as Node;
