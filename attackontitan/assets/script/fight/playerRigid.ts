...
export class PlayerRigid extends Component {
  ...
  onEnable() {
    clientEvent.on(
      constant.EVENT_TYPE.CHANGE_TO_INVINCIBLE_SPEED,
      this._changeToInvincibleSpeed,
      this
    )
    clientEvent.on(
      constant.EVENT_TYPE.RECOVERY_ORI_SPEED,
      this._recoveryOriSpeed,
      this
    )
    clientEvent.on(
      constant.EVENT_TYPE.CHANGE_TO_SPINE_SPEED,
      this._changeToSpineSpeed,
      this
    )
  }
  ...
  private _updateContactInfo() {
    ...
    if (this._grounded) {
      ...
      this._toSteep =
        this._groundContact.normal.y <=
        Math.cos((this.slopeLimit * Math.PI) / 180)
    } else {
