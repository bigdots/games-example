...
export class MapLoader extends Component {
  ...
  private _changendMapItemName() {
    ...
    for (let i = 0; i < ndMapItem.children.length; i++) {
      ndMapItem.children[i].name =
        ndMapItem.children[i].name.split('-')[0] + '-'
    }
    ...
    const write = () => {
      fs.writeFile(
        ...
        (err: Error) => {
          if (err) {
            console.warn(
              mapConstants.MAPLOADER_STRING.FAIL +
                ' ' +
                mapConstants.MAPLOADER_STRING.LEVEL +
                ':' +
                this.mapNameSave
            )
            ...
            console.warn(
              mapConstants.MAPLOADER_STRING.SUCCESS +
                ' ' +
                mapConstants.MAPLOADER_STRING.LEVEL +
                ':' +
                this.mapNameSave
    ...
    fs.exists(filePath, function (e: boolean) {
      if (!e) {
        fs.mkdir(filePath, function (err: Error) {
          if (err) {
            console.log(
              mapConstants.MAPLOADER_STRING.CREATE_DIRECTORY +
                ' ' +
                mapConstants.MAPLOADER_STRING.FAIL +
                ' ' +
                err
  ...
  private _loadMap() {
    ...
    projectPath.replace('\\', ' / ')
    let path =
      `${projectPath}/` + MAP_PATH + MAP_PREFIX + this.mapNameLoad + '.csv'
    const fs = require('fs')
  ...
  private _initEndRewardCircle(ndEnd: Node) {
    ...
    for (let i = 0; i < gameConstants.REWARD_CIRCLR_COUNT; i++) {
      const ndEndCircle = instantiate(
        ndMapItem.getChildByName(
          gameConstants.CSV_MAP_ITEM_NAME.ROAD_END_REWARD + '-'
        )!
      ) as unknown as Node
