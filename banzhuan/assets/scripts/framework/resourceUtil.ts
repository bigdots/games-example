...
export class resourceUtil {
  ...
  public static getMap(level: number, cb: Function) {
    ...
    this.loadRes(`map/config/${levelStr}`, null, (err: {}, txtAsset: any) => {
      ...
      if (txtAsset._file) {
        ...
        if (window['LZString']) {
          //@ts-ignore
          content = window['LZString'].decompressFromEncodedURIComponent(
            txtAsset._file
          )
        }
  ...
  public static getJsonData(fileName: string, cb: Function) {
    this.loadRes(
      'datas/' + fileName,
      null,
      function (err: any, content: JsonAsset) {
        if (err) {
          error(err.message || err)
          return
        }
  ...
  public static getTextData(fileName: string, cb: Function) {
    this.loadRes(
      'datas/' + fileName,
      null,
      function (err: any, content: TextAsset) {
        if (err) {
          error(err.message || err)
          return
        }
