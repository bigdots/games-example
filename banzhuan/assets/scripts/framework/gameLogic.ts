...
export class GameLogic {
  ...
  public static customEventStatistics(eventType: string, objParams?: any) {
    ...
    if (
      this.platform === 'cocos' &&
      window.cocosAnalytics &&
      window.cocosAnalytics.isInited()
    ) {
  ...
  public static vibrateShort() {
    let isEnableVibrate =
      StorageManager.instance.getGlobalData('vibration') ?? true
   ...
   * @memberof gameLogic
   */
  public static updatePayIcon(
    shareId: number,
    spIcon: SpriteComponent,
    callback?: Function,
    imgAd?: SpriteFrame,
    imgShare?: SpriteFrame
  ) {
  ...
  public static checkShowOfflineReward(target: any, manager: any) {
    ...
    uiManager.instance.showDialog('offlineReward/offlineReward', [
      target,
      seconds,
    ])
  }
  ...
  public static getTargetUnlockInfo(csvName: string, level: number) {
    let arrData = loadcsv.instance.getTableArr(csvName)
