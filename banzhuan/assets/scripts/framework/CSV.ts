...
@ccclass('CSV')
export class CSV {
  // static _instance: CSV

  // static get instance() {
  //   if (!this._instance) {
  //     this._instance = new CSV()
  //   }
  ...
  private STANDARD_ENCODE_OPTS = {
    delimiter: this.CELL_DELIMITERS[0],
    newline: this.LINE_DELIMITERS[0],
    skip: 0,
    limit: false,
    header: false,
  }
   ...
   * Utility Functions =======================
   * ========================================= */
  getterCast(value: any, index: any, cast: any, d: any) {
  ...
  map(collection: any, fn: Function) {
    var results = []
  ...
  getLimit(limit: any, len: any) {
    return limit === false ? len : limit
  }

  buildObjectConstructor(fields: any, sample: any, cast: any) {
    const getterCast = this.getterCast
  ...
  buildArrayConstructor(fields: any, sample: any, cast: any) {
    const getterCast = this.getterCast
    return function (d: any) {
  ...
  frequency(coll: any, needle: any, limit: any) {
  ...
  mostFrequent(coll: any, needles: any, limit: any) {
    var max = 0
    var detected
  ...
  unsafeParse(text: any, opts: any, fn: any) {
    ...
    if (opts.header) {
      ...
      constructor = this.buildObjectConstructor(
        fields,
        lines[0].split(opts.delimiter),
        opts.cast
      )
    } else {
      constructor = this.buildArrayConstructor(
        fields,
        lines[0].split(opts.delimiter),
        opts.cast
      )
  ...
  safeParse(text: any, opts: any, fn: Function) {
  ...
  encodeCells(line: any, delimiter: any, newline: any) {
  ...
  encodeArrays(coll: any, opts: any, fn: Function) {
    var delimiter = opts.delimiter
    var newline = opts.newline
    ...
    for (
      var cur = 0, lim = this.getLimit(opts.limit, coll.length);
      cur < lim;
      cur++
    ) {
  ...
  encodeObjects(coll: any, opts: any, fn: Function) {
    var delimiter = opts.delimiter
    var newline = opts.newline
    var header
    var row
    ...
    for (
      var cur = 1, lim = this.getLimit(opts.limit, coll.length);
      cur < lim;
      cur++
    ) {
      row = []
  ...
  public parse(text: any, opts: any, fn: Function) {
    var rows: any
    ...
    if (!opts.delimiter || !opts.newline) {
      ...
      opts.delimiter =
        opts.delimiter || this.mostFrequent(text, this.CELL_DELIMITERS, limit)
      opts.newline =
        opts.newline || this.mostFrequent(text, this.LINE_DELIMITERS, limit)
    }
  ...
  encode(coll: any, opts: any, fn: Function) {
    var lines: any
    ...
    return (
      (this.getType(coll[0]) === 'Array'
        ? this.encodeArrays
        : this.encodeObjects)(coll, opts, fn) &&
