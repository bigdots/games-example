...
export class CameraFollow extends Component {
  ...
  private _changeCameraType(type: number) {
    ...
    switch (type) {
      ...
      case gameConstants.CAMERA_TYPE_LIST.REWARD:
        ...
        this.offsetLookAtPos =
          gameConstants.CAMERA_REWARDROAD_DATA.offsetLookAtPos
        break
  ...
  lateUpdate(dt: number) {
    ...
    if (
      this.cameraType === gameConstants.CAMERA_TYPE_LIST.READY ||
      this.cameraType === gameConstants.CAMERA_TYPE_LIST.SWOOP
    )
      return //准备状态不需要持续修改坐标
    ...
    if (this.cameraType === gameConstants.CAMERA_TYPE_LIST.READY_TO_PLAYING) {
      ...
      if (
        GameManager.scriptPlayer.roleState ===
        gameConstants.ROLE_STATE_LIST.CLIMB
      ) {
        //攀爬动作特殊 特殊处理
        lerpPosNum = 0.02
        lerpEulNum = 0.02
  ...
  private followTarget(
    ...
    if (isAddBrickNum) {
      ...
      const brickNum =
        GameManager.scriptPlayer.brickNum > gameConstants.CAMERA_MAX_BRICKNUM
          ? gameConstants.CAMERA_MAX_BRICKNUM
          : GameManager.scriptPlayer.brickNum
      const brickOffset =
        gameConstants.CAMERA_ADDBRICK_OFFSET.clone().multiplyScalar(brickNum)
