...
export class Camera extends Component {
    ...
    private _onMouseWheel (event: EventMouse) {
        ...
        let pos = new Vec3(0, 0, wheel * 10);
        this.node.translate(pos, Node.NodeSpace.LOCAL);
    }

    update (deltaTime: number) {
        // Your update function goes here.

        let pos = new Vec3(this._horizontal * deltaTime * this._speed, 0, this._vertical * deltaTime * this._speed);
        this.node.translate(pos, Node.NodeSpace.LOCAL);
    }
}
