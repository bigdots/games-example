...
import { Size, UITransformComponent, Color, AnimationComponent, Vec3, ButtonComponent, find, profiler } from 'cc';
import { playerData } from './../../framework/playerData';
import { _decorator, Component, Node, EventTouch, Touch, LabelComponent, Label, SpriteFrame, SystemEventType, SpriteComponent, sp, UIComponent } from 'cc';
...
export class HomePanel extends Component {
    ...
    public lbDiamond: LabelComponent = null!;
    ...
    public spPayIconPowerHand: SpriteComponent = null!;

    @property(SpriteComponent)
    public spPayIconPowerKick: SpriteComponent = null!;
    ...
    private _refreshHandUpgradeBtn (isShowTip: boolean = true) {
        ...
        if (this._nextHandInfo) {
            ...
            if (this._nextHandInfo.handCost <  playerData.instance.playerInfo.diamond) {
                ...
                let lbComPrice = this.ndHandPrice.getChildByName("txt")?.getComponent(LabelComponent) as LabelComponent;
    ...
    private _refreshKickUpgradeBtn (isShowTip: boolean = true) {
        ...
        if (this._nextKickInfo) {
            ...
            if (this._nextKickInfo.kickCost <  playerData.instance.playerInfo.diamond) {
                ...
                let lbComPrice = this.ndKickPrice.getChildByName("txt")?.getComponent(LabelComponent) as LabelComponent;
    ...
    private _refreshLevel () {
        ...
        this.ndLevelList.children.forEach((item: Node, i: number, arr: any)=>{
            ...
            if (levelInfo) {
                let lbLevel = item.getChildByName("level")?.getComponent(LabelComponent) as LabelComponent;
                let spIcon = item.getChildByName("icon")?.getComponent(SpriteComponent) as SpriteComponent;
                let spLine = this.ndLineList.children[i].getComponent(SpriteComponent) as SpriteComponent;
                let spItemBg = item.getComponent(SpriteComponent) as SpriteComponent;
                let aniCom = item.getComponent(AnimationComponent) as AnimationComponent;
                ...
                if (levelInfo.type == constant.GAME_TYPE.NORMAL) {
                    ...
                    item.getComponent(UITransformComponent)?.setContentSize(56, 56);
                    ...
                    item.getComponent(UITransformComponent)?.setContentSize(74, 74);
                } else {
                    lbLevel.node.active = false;
                    spIcon.node.active = true;
