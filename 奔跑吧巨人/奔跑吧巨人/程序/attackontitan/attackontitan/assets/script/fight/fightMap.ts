...
import { _decorator, Component, Node, Vec3, RigidBodyComponent, Color, MeshRenderer, Material, find, AnimationComponent } from "cc";
...
export class FightMap extends Component {
    ...
    private _ndScoreLine: Node = null!;//道路模块里面的倍数跑道
    private _loadResLength: number = 20;//玩家当前位置z值往后20的模块需要加载
    ...
    private _initScoreLine () {
        if (GameManager.gameType === constant.GAME_TYPE.NORMAL) {
            // this._ndScoreLine = this.node.getChildByName("roadGroup")?.getChildByName("roadScore") as Node;
            ...
            resourceUtil.loadModelRes("roadScore/numItem").then((prefab: any)=>{
                for (let i = 0; i < BOARD_NUM; i++) {
                    ...
                    ndSingleDigitItem.setPosition(new Vec3(-0.006, 1.007, -0.242 - i * 0.5));
                    ndSingleDigitItem.setScale(new Vec3(0.01332, 0.528, 0.01595));
                ...
                for (let i = 0; i < BOARD_NUM; i++) {
                    ...
                    ndDecimal.setPosition(new Vec3(0.129, 1.007, -0.242 - i * 0.5));
                    ndDecimal.setScale(new Vec3(0.01332, 0.528, 0.01595));
                ...
                // FightMap.ndRoadScore.active = false;
            })
        }
    }
